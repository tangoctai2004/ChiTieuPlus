...
//    @State private var tabSelection: Int = 0 // 0 = ngày, 1 = tháng
//    
//    // MARK: - Colors
...
//    
//    // MARK: - Summary
//    private var totalIncome: Double { monthlyTransactions.filter { $0.type == "income" }.map(\.amount).reduce(0, +) }
//    private var totalExpense: Double { monthlyTransactions.filter { $0.type == "expense" }.map(\.amount).reduce(0, +) }
//    private var netBalance: Double { totalIncome - totalExpense }
//    
//    private var monthlyPieData: [CategorySummary] {
//        let grouped = Dictionary(grouping: monthlyTransactions.filter { $0.type == "expense" }) { $0.category?.name ?? "Khác" }
//        return grouped.map { CategorySummary(name: $0.key, total: $0.value.map(\.amount).reduce(0, +)) }
//    }
//    
//    private var dailyIncome: Double { dailyTransactions.filter { $0.type == "income" }.map(\.amount).reduce(0, +) }
//    private var dailyExpense: Double { dailyTransactions.filter { $0.type == "expense" }.map(\.amount).reduce(0, +) }
//    private var dailyNetBalance: Double { dailyIncome - dailyExpense }
//    
//    private var dailyPieData: [CategorySummary] {
//        let grouped = Dictionary(grouping: dailyTransactions.filter { $0.type == "expense" }) { $0.category?.name ?? "Khác" }
//        return grouped.map { CategorySummary(name: $0.key, total: $0.value.map(\.amount).reduce(0, +)) }
//    }
//    
...
//                            .padding(.top, 8)
//                        
//                        // Tab picker Ngày / Tháng
//                        Picker("", selection: $tabSelection) {
//                            Text("Theo Ngày").tag(0)
//                            Text("Theo Tháng").tag(1)
//                        }
